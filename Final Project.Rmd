...
correlation_test <- corr.test(final_df[, -c("ISO.code","Country", "Continent", "Regional.indicator")], method = "spearman", adjust = "none")
correlations <- cor(final_df[, -c("ISO.code","Country", "Continent", "Regional.indicator")], use = "complete.obs")
...
```{r Review of Correlations -- Visually and statistically, echo=TRUE, warning=FALSE, dpi=100, out.width="100%"}
filtered_dt <- final_df[, -c("Regional.indicator", "Population.growth", "Sex.ratio", "Suicide.rate", "ALCOHOL_AB", "TUBERC", "NTD", "DENTISTS", "PHARM")]

# Generate a Correlation Matrix in addition to histograms and star symbols that represent p-value significance
  library("PerformanceAnalytics")
  chart.Correlation(filtered_dt[, -c("ISO.code","Country", "Continent")], histogram=T, pch=19, method = c("spearman"))
```
...
# Open a PDF device
pdf("/Users/ntham/Library/CloudStorage/OneDrive-Personal/School/Senior/Spring/Statistics in Biomedical Informatics (B518)/Project/B518_Final_Project/plots2.pdf", width = 25, height = 15)  # Adjust width and height as needed
...
```{r}
# Load the PerformanceAnalytics library
library(PerformanceAnalytics)

# Subset the data to remove unwanted columns
filtered_dt <- final_df[, -c("Regional.indicator", "Population.growth", "Sex.ratio", "Suicide.rate", "ALCOHOL_AB", "TUBERC", "NTD", "DENTISTS", "PHARM")]

# Generate scatter plots
chart.Correlation(filtered_dt[, -c("ISO.code","Country", "Continent")], plotMethod = "scatter")
